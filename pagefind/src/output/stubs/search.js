...
class Pagefind {
    ...
    async loadMeta() {
        ...
        let compressed_meta = await fetch(`/_pagefind/pagefind.pf_meta?ts=${Date.now()}`);
        compressed_meta = await compressed_meta.arrayBuffer();
        this.searchMeta = gunzip(new Uint8Array(compressed_meta));
    }

    async loadWasm() {
        let compressed_wasm = await fetch("/_pagefind/wasm.pagefind");
    ...
    async loadChunk(hash) {
        ...
        let compressed_chunk = await fetch(`/_pagefind/index/${hash}.pf_index`);
    ...
    async loadFragment(hash, excerpt = [0, 0], locations = []) {
        let fragment = await fetch(`/_pagefind/fragment/${hash}.pf_fragment`);
