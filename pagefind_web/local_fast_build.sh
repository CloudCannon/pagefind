...
if [ "$1" = "debug" ]; then
    ...
    RUSTUP_TOOLCHAIN=nightly wasm-pack build --release -t no-modules --features en --manifest-path ./Cargo.toml -Z build-std=panic_abort,std -Z build-std-features=panic_immediate_abort
fi
